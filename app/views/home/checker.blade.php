    ...
    <table class="table table-condensed table-bordered table-striped table-hover check-table">
      ...
      if ($sites AND count($sites) > 0) {
          foreach ($sites AS $site) {
              ?>
        <tr>
          <td><?= ($url == $site['url']) ? "<strong style='color:red'>{$site['url']}</strong>" : "<a href='/checker?uid={$site['id']}&filter_status[]=".implode('&filter_status[]=', $filter_status)."&page=".Input::get('page',1)."'>{$site['url']}</a>" ?>
    ...
    <form action="/save-data" method="POST" style="margin: 0;">
        ...
        <div>
          ...
          <div style="display: inline-block;">
            <div id="check-phones-content" style="font-size: 11px; line-height: 11px;"></div>
            <div id="check-emails-content" style="font-size: 11px; line-height: 11px;"></div>
          </div>
          </div>
          <div style="padding: 10px 0;">
              ...
              ?>
              <button class="btn btn-inverse btn-small">Сохранить и далее</button>
          </div>
        </div>
    </form>
...
function setIframeHeight(){
    $('#load-container').height( $(document).height() - 216 );
}
